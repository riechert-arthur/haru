...
export default function SkillList() {
  ...
  return (
    <div className="w-full">
      <text
        className={`${montserrat.className} text-xl tracking-wider font-light`}
      >
        <i>Click each skill for more detail.</i>
      </text>
            ...
            >
              <div className="flex overflow-x-scroll overflow-y-hidden">
                ...
                <Image
                  ...
                  width="50"
                />
                <h2 className="ml-6 leading-loose translate-y-1">
                  {skill.Name}
                </h2>
              </div>
              <div
                className={`mt-6 ${montserrat.className} ${skill.Expanded ? "" : "hidden"}`}
              >
                <text
                  className={`text-3xl ${skill.Details.Software ? "" : "hidden"}`}
                >
                  Software
                </text>
                <ul className="pl-2 ml-1 border-l">
                  {skill.Details.Software?.map(
                    (software: string, i: number) => {
                      return (
                ...
                <text
                  className={`text-3xl ${skill.Details.Libraries ? "" : "hidden"}`}
                >
                  Libraries
                </text>
                <ul className="pl-2 ml-1 border-l">
                  {skill.Details.Libraries?.map(
                ...
                <text
                  className={`text-3xl ${skill.Details.Frameworks ? "" : "hidden"}`}
                >
                  Frameworks
                </text>
                <ul className="pl-2 ml-1 border-l">
                  {skill.Details.Frameworks?.map(
                    (framework: string, i: number) => {
                      return (
                        <li key={i} className="text-2xl font-light">
                          {framework}
                        </li>
